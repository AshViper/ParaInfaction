...
public abstract class ParainfectionBaseEntity extends Monster implements GeoEntity {
    ...
    public void awardKillScore(Entity killed, int score, DamageSource source) {
        ...
        if (!this.level().isClientSide && killed instanceof LivingEntity) {
            ...
            // 倒されたエンティティの種類に応じて召喚するエンティティを変更
            if (killed.getType() == EntityType.COW || killed.getType() == EntityType.MOOSHROOM) {
                summonType = ParainfectionEntities.CORRODEDCOW.get();
            } else if (killed.getType() == EntityType.SPIDER) {
                summonType = ParainfectionEntities.GNAWLING.get();
            } else if (killed.getType() == EntityType.PIG) {
                summonType = ParainfectionEntities.CORRODEDPIG.get();
            } else if (killed.getType() == EntityType.SHEEP) {
                summonType = ParainfectionEntities.CORRODEDSHEEP.get();
            } else if (killed.getType() == EntityType.CHICKEN) {
                summonType = ParainfectionEntities.CORRODEDCHIKEN.get();
            } else if (killed.getType() == EntityType.ZOMBIE) {
                summonType = ParainfectionEntities.CORRODEDZOMBIE.get();
            } else {
                summonType = ParainfectionEntities.INCOMPLETEFIELD.get();
            }
    ...
    public void die(DamageSource source) {
        ...
        if (!this.level().isClientSide) {
            ...
            if (random.nextFloat() < 0.2f) {
                EntityType<?> summonType = ParainfectionEntities.NESTERRA1.get();
                LivingEntity newEntity = (LivingEntity) summonType.create(this.level());
